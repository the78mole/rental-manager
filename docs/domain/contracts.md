...
```mermaid
graph TD
    Apartment --> M1["Electricity Meter\nunit: kWh"]
    Apartment --> M2["Gas Meter\nunit: m³"]
    Apartment --> M3["Cold Water\nunit: m³"]
    Apartment --> M4["Hot Water\nunit: m³"]

    M1 --> R1["Reading 01.01.2025\n12340 kWh"]
    M1 --> R2["Reading 01.04.2025\n12895 kWh"]
    M1 --> R3["Reading 01.07.2025\n13210 kWh"]
    M1 --> R4["Reading 01.10.2025\n13680 kWh"]
    M1 --> R5["Reading 01.01.2026\n14200 kWh"]
```
...
```mermaid
flowchart TD
    Submit["POST /tenant/meter-readings\n{meter_id, value, date}"] --> V1{"value > previous\nreading?"}
    V1 -->|No| Err1["400 Bad Request\nValue must be ascending"]
    V1 -->|Yes| V2{"Tenant has active\ncontract for this\napartment?"}
    V2 -->|No| Err2["403 Forbidden"]
    V2 -->|Yes| Save["Store reading\n201 Created"]
    Save --> Notify["Notify landlord\n(optional webhook)"]
```
...
```mermaid
flowchart TD
    Trigger["POST /landlord/billing/run\n{period_start, period_end}"] --> Task["Celery task\ndispatched"]

    Task --> FetchContracts["Fetch active contracts\nfor billing period"]
    FetchContracts --> PerContract["For each contract"]

    PerContract --> Readings["Fetch meter readings\nstart + end of period"]
    Readings --> Consumption["Calculate consumption\nper meter type"]
    Consumption --> Tariff["Apply tariffs\n+ service charges"]
    Tariff --> Share["Calculate tenant's\nproportion (if WG)"]
    Share --> CreateBill["Create BILL record\n+ PDF generation"]

    CreateBill --> Done["All bills stored\nStatus: PENDING → SENT"]
```
