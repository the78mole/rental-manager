...
```mermaid
flowchart TD
    Request["GET /landlord/buildings"] --> Auth["JWT validated\nrole = caretaker"]
    Auth --> Filter["Apply caretaker scope filter:\nOnly buildings with assignment\nfor this caretaker_id"]
...
sequenceDiagram
    ...
    Admin->>Admin: POST /admin/caretakers\n(creates Keycloak user + DB record)
    Admin-->>Caretaker: Share credentials (email)

    Landlord->>Landlord: POST /landlord/buildings/{id}/caretakers/{id}\n(assigns caretaker to building)
