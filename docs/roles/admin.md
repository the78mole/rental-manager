...
graph LR
    ...
    A -->|"can manage"| A
    A -->|"can manage"| OP
    OP -->|"NO user mgmt"| note["Landlord &\nTenant management only"]
```
...
```mermaid
flowchart TD
    Admin["ADMIN or OPERATOR"] --> Form["Fill out landlord form\n/admin/landlords/new"]
    Form --> KC["User created in Keycloak\n(realm role: landlord)"]
    KC --> DB["User synced to local DB\n(role: LANDLORD)"]
    DB --> Email["Welcome e-mail sent\n(optional)"]
    Email --> Login["Landlord logs in\nand sets up portfolio"]
```
...
```mermaid
flowchart TD
    Admin["ADMIN / OPERATOR"] --> CreateUser["POST /admin/caretakers\nCreate user in Keycloak\n(role: caretaker)"]
    CreateUser --> Assign["Landlord assigns caretaker\nto building or apartment"]
    Assign --> Access["Caretaker can now access\nassigned objects in /landlord/*"]
```
